...
begin
  ...
  values (
    ...
    v_payment_status,
    v_order_status,
    case when v_payment_status = 'paid'::public.payment_status then 'paid'::public.status else 'pending'::public.status end,
    v_user_id,
    v_highest_offer_id,
    v_discount_total
  );
...
begin
  ...
  set
    ...
    status = case
      when p_payment_status = 'paid'::public.payment_status then 'paid'::public.status
      when v_order_status = 'cancelled'::public.order_status then 'cancelled'::public.status
      when v_order_status = 'shipped'::public.order_status then 'shipped'::public.status
      when v_order_status = 'delivered'::public.order_status then 'delivered'::public.status
      when v_order_status = 'confirmed'::public.order_status then 'confirmed'::public.status
      else 'pending'::public.status
    end,
