...
function ProductPage() {
  ...
  const handleAdd = () => {
    ...
    addItem({
      id: item.id, name: item.name, price_egp: item.price_egp, image_url: item.image_url,
  ...
  return (
    <Layout>
      <div className="px-6 md:px-12 pt-6 max-w-[1600px] mx-auto" style={{ fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: "#9a9a9a" }}>
        <Link to="/" style={{ color: "#9a9a9a" }}>Home</Link>
        <span style={{ margin: "0 8px" }}>/</span>
        <Link to="/shop" style={{ color: "#9a9a9a" }}>Shop</Link>
      ...
      <div className="grid md:grid-cols-[60%_40%] gap-0 max-w-[1600px] mx-auto mt-6">
        ...
        <div className="p-6 md:p-12 flex flex-col">
          ...
          {soldOut ? (
            ...
            <button className="jb-btn w-full mt-4" onClick={handleAdd}>Add to bag</button>
          )}
        </div>
      </div>
