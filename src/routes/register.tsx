  ...
  return (
    <Layout>
      <div className="max-w-md mx-auto px-6 py-16">
        ...
        <h1 style={{ fontSize: "clamp(1.75rem, 4vw, 2.25rem)", fontWeight: 300, marginTop: 6, color: "#fff" }}>Create account</h1>
        <form onSubmit={submit} className="mt-8 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="jb-label">First name</label>
              <input className="jb-input" value={firstName} onChange={(e) => setFirstName(e.target.value)} />
            </div>
            <div>
              <label className="jb-label">Last name</label>
              <input className="jb-input" value={lastName} onChange={(e) => setLastName(e.target.value)} />
            </div>
          </div>
          <div>
          ...
          <button type="submit" disabled={busy} className="jb-btn w-full">{busy ? "Creating…" : "Create account"}</button>
        </form>
        <button onClick={googleSignup} className="jb-btn-ghost w-full mt-3">Continue with Google</button>
        <div className="mt-6 text-center" style={{ fontSize: 12, letterSpacing: "0.15em", textTransform: "uppercase", color: "#9a9a9a" }}>
          Already have an account? <Link to="/login" className="hover:underline" style={{ color: "#fff" }}>Log in</Link>
        </div>
      </div>
    </Layout>
  );
